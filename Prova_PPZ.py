...
# loja_tinta(109) -> 3
def loja_tinta(m):
  return m // 54 if 54 * (m // 54) == m else m // 54 + 1
...
def alunos_problema(a_sorri, b_sorri):
  return True if a_sorri == b_sorri else False
...
def papagaio(falando, hora):
  return True if falando and (hora < 7 or hora > 20) else False
...
def pego_correndo(speed, is_birthday):
  if (is_birthday and speed <= 65) or speed <= 60:
    ...
    return 1 if speed > 65 and speed <= 85 else 2
  return 1 if speed > 60 and speed <= 80 else 2
...
def count_code(s):
  i = 0
  for letra in map(chr, range(97, 123)):
    i += s.count('co' + letra + 'e')
  return i
