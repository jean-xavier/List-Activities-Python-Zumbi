...
def same_first_last(nums):
  return True if len(nums) and nums[0] == nums[-1] else False
...
def sum2(nums):
  if not nums: 
    return 0
  return nums[0] + nums[1] if len(nums) > 1 else nums[0] + 0
...
def middle_way(a, b):
  ...
  meio_a = len(a) // 2
  meio_b = len(b) // 2
  list_meio.append(a[meio_a])
  list_meio.append(b[meio_b])
  return list_meio
...
def pego_correndo(speed, is_birthday):
  if speed <= 60:
    ...
    if speed <= 65:
      return 0
    return 1 if speed <=85 else 2
  return 1 if speed <=80 else 2
