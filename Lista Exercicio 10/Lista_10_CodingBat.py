...
# near_ten(19) -> True
def near_ten(n):
  for multiplos in range(0, n+10, 10):
    if (n - 2 == multiplos) or (n - 1 == multiplos) or (n + 2 == multiplos) or (n + 1 == multiplos) or (n == multiplos):
      return True
  return False
...
def cat_dog(s):
  return True if s.count('') and s.count('cat') == s.count('dog') else False
...
def end_other(a, b):
  return True if a[-len(b)::].lower() == b.lower() or b[-len(a)::].lower() == a.lower() else False
...
def count_evens(nums):
  num_pares = 0
  for num in nums:
    num_pares = num_pares + 1 if num % 2 == 0 else num_pares 
  return num_pares
...
# has22([2, 1, 2]) -> False
def has22(nums):
  for contador in range(1, len(nums)):
    if nums[contador - 1] == nums[contador]:
      return True
  return False
...
def soma_na_lista(n, lista):
  for i in range(len(lista)):
    for x in range(len(lista)):
      if lista[i] + lista[x] == n and lista[i] != lista[x]:
        return True
  return False
...
def fila_tijolos(n_peq, n_gra, meta):
  if n_peq + (n_gra * 5) >= meta and (n_peq + (n_gra * 5)) - meta <= n_peq:
    return True
  return False
